...
//      'sdk_version': _opts.sdkVersion
  ...
  function _executeRequestApi2 (path, query, datapost, contentType) {
    return login().then(function () {
      ...
      _.defaults(headers, _opts.customHeaders, basicHeaders);
  ...
  /**
   * Returns complete and request-ready options object for splitDeliveryData.
   * @param pkg
   * @param versionCode
   * @param fileIndex
   */
  function getSplitDeliveryDataCompleteDownloadInfo (pkg, versionCode, fileIndex) {
    const headers = {
      'User-Agent': _opts.downloadUserAgent,
      'Accept-Encoding': ''
    };
    ...
    return getDownloadInfo(pkg, versionCode)
      .then(function (res) {
        if (fileIndex > res.splitDeliveryData.length) {
          throw new Error('no such splitDeliveryData at index');
        }
        assert(res.splitDeliveryData[fileIndex], 'require downloadUrl');
        assert(res.downloadAuthCookie, 'require downloadAuthCookie');
        const url = res.splitDeliveryData[fileIndex].downloadUrl;
        const cookieJar = _prepCookies(url, res.downloadAuthCookie);
        return { url: url, jar: cookieJar, headers: headers };
      });
  }
  ...
  /**
   * Download splitDeliveryData for a specific package, at a specific versionCode.
   * @return {Promise} promise of request object, e.g., can use .pipe(..)
   */
  function downloadSplitDeliveryData (pkg, versionCode, index) {
    return getSplitDeliveryDataCompleteDownloadInfo(pkg, versionCode, index)
      .then(function (options) {
        return _request.get(options);
      });
  }
  ...
  return {
    ...
    downloadSplitDeliveryData: function download (pkg, vc, idx, cb) {
      return downloadSplitDeliveryData(pkg, vc, idx).nodeify(cb);
    },
